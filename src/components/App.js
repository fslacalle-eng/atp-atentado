...
const tabs=[["standings","Clasif."],["matches","Partidos"+(pendM.length+dispM.length>0?" ("+String(pendM.length+dispM.length)+")":"")],["simulator","Simulador"],["tournaments","Torneos"],["challenges","Retos"+(pendCh.length>0?" ("+String(pendCh.length)+")":"")],["h2h","H2H"],["hof","Hall of Fame"],["forum","Foro"],["rules","Reglas"],...(isA?[["admin","Admin"+(S.pending.length>0?" ("+String(S.pending.length)+")":"")]]:[])]
const cfg=S.config;
  ...
  return(<div style={{background:W.card+"ee",borderRadius:12,padding:16,border:"1px solid "+col+"33",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",gap:8}}>
    ...
    {customBody?customBody:(!hasData?(<div style={{textAlign:"center",padding:"12px 0",color:W.cream3,fontSize:12,fontStyle:"italic"}}>
      ...
      {!isTie?(<div style={{display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:14,fontWeight:600,color:W.cream,cursor:"pointer"}} onClick={()=>{const pl=pO(playerIds[0]);if(pl)setModal({type:"profile",player:pl});}}>{pSh(playerIds[0])}</div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:6}}>
          {playerIds.slice(0,5).map(pid=>(<div key={pid} style={{textAlign:"center",cursor:"pointer"}} onClick={()=>{const pl=pO(pid);if(pl)setModal({type:"profile",player:pl});}}>
            ...
            <div style={{fontSize:10,color:W.cream,fontWeight:600,marginTop:2,maxWidth:60,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{pSh(pid)}</div>
          </div>))}
  ...
  return(<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}>
      <HofCard icon="✅" title="Más victorias" subtitle="Partidos regulares y retos ganados"
        help="Número total de partidos ganados en la historia de la liga. Incluye partidos regulares (Liga y Amistoso) y retos. No incluye partidos de torneo. Puedes filtrar por superficie para ver quién domina en cada tipo de pista."
      ...
      <HofCard icon="🔥" title="Mejor racha de victorias" subtitle="Partidos ganados consecutivamente"
        help="Mayor número de partidos ganados de forma consecutiva (sin ninguna derrota entre medias). Solo cuentan partidos regulares y retos (no torneos). Se calcula mirando toda la historia del jugador, no solo su racha actual. En caso de empate se muestran todos los jugadores con la misma racha récord."
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}>
      <HofCard icon="🎯" title="Más partidos jugados" subtitle="Regulares y retos, todas las superficies"
        help="El jugador más activo de la liga. Suma partidos regulares y retos sobre todas las superficies. No incluye partidos de torneo. En caso de empate aparecen todos los que compartan el número máximo de partidos."
      ...
      <HofCard icon="⚔️" title="Enfrentamiento más repetido" subtitle="La rivalidad por excelencia"
        help="El par de jugadores que más veces se han enfrentado en partidos regulares y retos (no cuentan los torneos). Se muestra el head-to-head entre ambos: cuántas victorias tiene cada uno en esa rivalidad. En caso de empate entre varios pares con el mismo número de enfrentamientos, se indica al final de la tarjeta."
        customBody={(()=>{
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:12}}>
      <HofCard icon="❌" title="Más derrotas" subtitle="El que más ha probado el polvo de la pista"
        help="Número total de partidos perdidos. Incluye partidos regulares y retos; no incluye torneos. Filtrable por superficie. El hecho de aparecer aquí no implica ser peor jugador — suele correlacionar simplemente con jugar mucho."
      ...
      <HofCard icon="🧊" title="Peor racha de derrotas" subtitle="Partidos perdidos consecutivamente"
        help="El mayor número de partidos perdidos de forma consecutiva (sin ninguna victoria de por medio) que ha tenido un jugador en toda su historia. Solo cuentan partidos regulares y retos, no torneos. Ser «líder» en esta categoría no implica nada — todos pasamos por rachas malas."
    ...
    <div style={{marginTop:24,padding:14,background:W.darkGreen,borderRadius:8,border:"1px solid "+W.gold+"22",fontSize:11,color:W.cream3,fontStyle:"italic",textAlign:"center"}}>
      Los datos se recalculan en tiempo real desde los partidos registrados. Los torneos no cuentan para estadísticas de partidos regulares ni retos. Los valores "Sorpassos" y "Upset" usan el umbral dinámico definido por la base de puntuación y el bonus de equidad configurados.
    </div>
  </div>);
}
