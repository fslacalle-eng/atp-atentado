...
const tabs=[["standings","Clasif."],["matches","Partidos"+(pendM.length+dispM.length>0?" ("+String(pendM.length+dispM.length)+")":"")],["simH2H","Simulador / H2H"],["tournaments","Torneos"],["challenges","Retos"+(pendCh.length>0?" ("+String(pendCh.length)+")":"")],["trophies","Trofeos"],["forum","Foro"],...(isA?[["admin","Admin"+(S.pending.length>0?" ("+String(S.pending.length)+")":"")]]:[])]
const cfg=S.config;
...
<nav style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:20}}>{tabs.map(([id,l])=>(<button key={id} style={{...snv(view===id),position:"relative"}} onClick={()=>setView(id)}>{l}{id==="forum"&&forumHasNew&&view!=="forum"&&<span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:W.danger,marginLeft:6,verticalAlign:"middle",boxShadow:"0 0 6px "+W.danger}}/>}</button>))}</nav>
  ...
  {(()=>{
    ...
    const doClose=(kind)=>{
      ...
      flash(record.championId?"Temporada cerrada — Campeón: "+pSh(record.championId):"Temporada cerrada sin campeón");
    };
  ...
  <div style={{marginTop:32,paddingTop:16,borderTop:"1px solid "+W.gold+"22",textAlign:"center",fontSize:11,color:W.cream3}}>
    <button style={{background:"none",border:"none",color:W.gold,cursor:"pointer",fontSize:11,textDecoration:"underline",padding:4}} onClick={()=>setModal({type:"rules"})}>Ver reglas</button>
    <span style={{margin:"0 8px",opacity:.5}}>·</span>
    <span style={{opacity:.7}}>ATP Atentado</span>
  </div>
</div>
);
...
function SimH2HView(){
  ...
  return(<div>
    <div style={{display:"flex",gap:6,marginBottom:14}}>
      <button style={{...snv(sub==="simulator"),flex:1,padding:"10px"}} onClick={()=>setSub("simulator")}>🎯 Simulador</button>
      <button style={{...snv(sub==="h2h"),flex:1,padding:"10px"}} onClick={()=>setSub("h2h")}>📊 H2H</button>
    </div>
    {sub==="simulator"&&<SimulatorView/>}
    {sub==="h2h"&&<H2HView/>}
  </div>);
}
...
function TrophiesView(){
  const[sub,setSub]=useState("hof");
  return(<div>
    <div style={{display:"flex",gap:6,marginBottom:14}}>
      <button style={{...snv(sub==="hof"),flex:1,padding:"10px"}} onClick={()=>setSub("hof")}>🏛️ Hall of Fame</button>
      <button style={{...snv(sub==="seasons"),flex:1,padding:"10px"}} onClick={()=>setSub("seasons")}>🏆 Temporadas</button>
    </div>
    {sub==="hof"&&<HallOfFameView/>}
    {sub==="seasons"&&<SeasonsView/>}
  </div>);
}
...
function SanctionForm(){
  ...
  return(<div>
    ...
    <button style={{...sbt("danger"),width:"100%"}} onClick={()=>{
      ...
      setModal(null);
      flash(publish?"Sanción aplicada y publicada":"Sanción aplicada");
    }}>Aplicar</button>
  </div>);
}
function BonusForm(){
  ...
  return(<div>
    ...
    <button style={{...sbt("primary"),width:"100%"}} onClick={()=>{
      ...
      setModal(null);
      flash("+"+String(amt)+" pts a "+pSh(pid)+(publish?" (publicado)":""));
    }}>Otorgar</button>
  </div>);
}
