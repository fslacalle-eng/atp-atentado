 ...
 * Bonus based on points difference alone.
 * Applies whenever the winner is worse-ranked than the loser by at least
 * (equityMatchesGap * eloBase) points, regardless of whether they had
 * previously faced each other.
 * Bonus is eloBase * (1 + equityBonusPct/100), rounded DOWN.
 *
 * Note: the 6th parameter (hasH2H) used to gate the bonus on a previous
 * encounter. It is kept in the signature for backwards compatibility with
 * callers but is now ignored — the bonus applies from the first match too.
 */
export const calcPts = (winnerPts, loserPts, base, bonusPct, matchesGap, _hasH2H) => {
  ...
  matches.forEach((m) => {
    ...
    if (m.countsForStandings) {
      ...
      const awarded = calcPts(
        ...
        true // hasH2H parameter retained for signature compatibility; no longer gates the bonus
      );
...
/* ===== SEASONS =====
 * Calendar-aligned half-year seasons (Jan–Jun and Jul–Dec).
 * Each season is identified by { year, half: 1|2 }.
 * A stored closed season lives in S.seasons with { id, year, half, startDate,
 * endDate, closed: true, championId, standings: [{playerId, netPts, wins}] }.
 *
 * Live season: always computed on the fly from the current date.
 *
 * Design decisions:
 * - Metric: NET balance within window = points gained (match wins, equity
 *   bonuses, tournament position points, admin bonuses) MINUS points lost
 *   (challenge losses, inactivity decay, admin sanctions) within the window.
 * - No floor-at-zero clamping on the season score (a player can finish a
 *   season with negative net points). This is intentional: it reflects
 *   genuinely tough seasons. The UI still shows them, just with a negative sign.
 * - Matches are attributed to a season by their `date` field, not registration.
 * - Tournament position points go to the season where the FINAL was confirmed.
 * - The first season may start at a date before any matches; players with
 *   zero activity in a season simply don't appear on the leaderboard.
 */

export const seasonBoundsForDate = (ts) => {
  const d = new Date(ts);
  const y = d.getFullYear();
  const month = d.getMonth(); // 0-indexed
  const half = month < 6 ? 1 : 2;
  const start = new Date(y, half === 1 ? 0 : 6, 1, 0, 0, 0, 0).getTime();
  const end =
    half === 1
      ? new Date(y, 6, 1, 0, 0, 0, 0).getTime() - 1
      : new Date(y + 1, 0, 1, 0, 0, 0, 0).getTime() - 1;
  return { year: y, half, start, end };
};

export const seasonIdFor = (year, half) => "s-" + String(year) + "-h" + String(half);

export const seasonNameFor = (year, half) =>
  "Temporada " + String(year) + (half === 1 ? " · 1er semestre" : " · 2º semestre");

export const getCurrentSeasonInfo = (S, nowTs) => {
  const ts = nowTs || Date.now();
  const b = seasonBoundsForDate(ts);
  const id = seasonIdFor(b.year, b.half);
  const stored = (S.seasons || []).find((s) => s.id === id);
  return {
    id,
    year: b.year,
    half: b.half,
    startDate: b.start,
    endDate: b.end,
    name: seasonNameFor(b.year, b.half),
    closed: stored ? !!stored.closed : false,
  };
};

/**
 * Compute a season's leaderboard (net balance within the window).
 * Returns [{ playerId, nickname, netPts, gained, lost, wins, losses }, ...]
 * sorted by netPts descending.
 */
export const computeSeasonStandings = (S, startDate, endDate) => {
  if (!S) return [];
  const cfg = S.config || {};
  const tally = {};
  const ensure = (pid) => {
    if (!tally[pid]) tally[pid] = { playerId: pid, gained: 0, lost: 0, wins: 0, losses: 0 };
    return tally[pid];
  };

  // === MATCH POINTS ===
  // Rebuild the global points timeline to know the points of each player AT
  // the time of each match (so equity bonuses are computed correctly). We
  // reuse computePointsAtDate per-match, but to avoid O(n²) we compute the
  // running standings ourselves by chronological replay.
  const allMatches = (S.matches || [])
    .filter((m) => !m.annulled && m.status === "confirmed" && m.winner)
    .sort((a, b) => a.date - b.date);

  const running = {}; // pid → current points (history-to-date)
  (S.players || []).forEach((p) => { running[p.id] = 0; });

  const inWindow = (ts) => ts >= startDate && ts <= endDate;

  for (const m of allMatches) {
    const winner = m.winner;
    const loser = m.winner === m.player1 ? m.player2 : m.player1;
    if (running[winner] === undefined) running[winner] = 0;
    if (running[loser] === undefined) running[loser] = 0;

    if (m.tournamentId) {
      // Tournament match: wins/losses count, but points are assigned
      // separately via computeTournamentPts at tournament close. No per-match
      // delta on `running`.
      if (inWindow(m.date)) {
        ensure(winner).wins++;
        ensure(loser).losses++;
    ...
    // Regular / challenge match
    if (m.countsForStandings) {
      const winnerPts = running[winner];
      const loserPts = running[loser];
      const awarded = calcPts(
        winnerPts,
        loserPts,
        cfg.eloBase,
      ...
      running[winner] += awarded;
      if (inWindow(m.date)) {
        ensure(winner).gained += awarded;
        ensure(winner).wins++;
        ensure(loser).losses++;
      }
    } else if (inWindow(m.date)) {
      // Friendly match (no points) — still counts as a win/loss for season stats
      ensure(winner).wins++;
      ensure(loser).losses++;
    }

    if (m.isChallenge) {
      const base = cfg.eloBase;
      // Loser pays the base fee — and we track it in the season window
      const before = running[loser];
      running[loser] = Math.max(0, running[loser] - base);
      const actualLoss = before - running[loser];
      if (inWindow(m.date)) {
        ensure(loser).lost += actualLoss;
      }
    }
  }

  // === TOURNAMENT POSITION POINTS ===
  // Assign tournament position points to the season in which the final was
  // confirmed. We use finalConfirmedAt if present, else fall back to the last
  // match date of that tournament's bracket.
  for (const t of S.tournaments || []) {
    if (!t.bracket) continue;
    const f = t.bracket.final && t.bracket.final[0];
    if (!f || !f.winner || f.status !== "confirmed") continue;
    const closeTs = t.finalConfirmedAt || f.date || endDate;
    if (!inWindow(closeTs)) continue;
    const ranks = rankTournamentPlayers(t);
    ranks.forEach((r) => {
      const pts = computeTournamentPts(r.pos, t.players.length, cfg);
      if (pts > 0) ensure(r.playerId).gained += pts;
    });
  }

  // === INACTIVITY DECAY ===
  // Replay inactivity inside the window. Cap at "now" if the season is still
  // in progress (we can't penalize someone for inactivity in the future).
  const simHorizon = Math.min(endDate, Date.now());
  for (const p of S.players || []) {
    ...
    const last = pms[pms.length - 1].date;
    if (last >= startDate && last < simHorizon) {
      const periods = Math.floor(
        (simHorizon - last) / (cfg.inactDays * 86400000)
      );
      // Approximate: use running points after last activity as base
      let simPts = running[p.id] || 0;
      for (let i = 0; i < periods; i++) {
        const penalty = Math.floor(simPts * (cfg.inactPct / 100));
        const actual = Math.min(penalty, simPts);
        simPts = Math.max(0, simPts - penalty);
        ensure(p.id).lost += actual;
      }
    }
  }

  // === ADMIN SANCTIONS & BONUSES ===
  for (const p of S.players || []) {
    for (const s of p.sanctions || []) {
      if (!s.date || !inWindow(s.date)) continue;
      if (s.type === "bonus") ensure(p.id).gained += s.amount || 0;
      else ensure(p.id).lost += s.amount || 0;
    }
  }
  ...
  const rows = Object.values(tally).map((r) => ({
    ...r,
    netPts: r.gained - r.lost,
  }));

  // Annotate with nickname for convenience
  for (const r of rows) {
    const p = (S.players || []).find((x) => x.id === r.playerId);
    r.nickname = p ? p.nickname : "?";
  }
...
/**
 * Helper: rank a tournament's players by final position.
 * Returns [{ playerId, pos }] with pos = 1 for champion.
 */
const rankTournamentPlayers = (t) => {
  const out = [];
  const bk = t.bracket || {};
  const used = new Set();
  const winnerOf = (m) => (m && m.status === "confirmed" ? m.winner : null);
  const loserOf = (m) => {
    if (!m || m.status !== "confirmed" || !m.winner) return null;
    return m.winner === m.player1 ? m.player2 : m.player1;
  };
  // Final: champion pos=1, runner-up pos=2
  if (bk.final && bk.final[0]) {
    const f = bk.final[0];
    const w = winnerOf(f);
    const l = loserOf(f);
    if (w) { out.push({ playerId: w, pos: 1 }); used.add(w); }
    if (l) { out.push({ playerId: l, pos: 2 }); used.add(l); }
  }
  // Semis losers: pos=3
  if (bk.semis) {
    for (const sm of bk.semis) {
      const l = loserOf(sm);
      if (l && !used.has(l)) { out.push({ playerId: l, pos: 3 }); used.add(l); }
    }
  }
  // Quarters losers: pos=5
  if (bk.quarters) {
    for (const qm of bk.quarters) {
      const l = loserOf(qm);
      if (l && !used.has(l)) { out.push({ playerId: l, pos: 5 }); used.add(l); }
    }
  }
  // R16 losers: pos=9
  if (bk.r16) {
    for (const rm of bk.r16) {
      const l = loserOf(rm);
      if (l && !used.has(l)) { out.push({ playerId: l, pos: 9 }); used.add(l); }
    }
  }
  // Remaining players (league-phase eliminated): pos=pos of league
  for (const p of (t.players || [])) {
    if (!used.has(p)) { out.push({ playerId: p, pos: 17 }); }
  }
  return out;
};

/**
 * Close a season: given a season info object (id/year/half/dates), compute
 * its final standings and return a season record ready to append to S.seasons.
 * Does NOT mutate S. The caller should update state with [...S.seasons, rec].
 */
export const buildClosedSeasonRecord = (S, seasonInfo) => {
  const standings = computeSeasonStandings(S, seasonInfo.startDate, seasonInfo.endDate);
  const championId = standings.length > 0 ? standings[0].playerId : null;
  return {
    id: seasonInfo.id,
    year: seasonInfo.year,
    half: seasonInfo.half,
...
/**
 * Map of all champion IDs across closed seasons → array of season records where
 * they were champion. Useful for the permanent 🏆 badge.
 */
export const getAllChampions = (S) => {
  const map = {};
  for (const s of S.seasons || []) {
    if (!s.closed || !s.championId) continue;
    if (!map[s.championId]) map[s.championId] = [];
    map[s.championId].push(s);
  }
  return map;
};

export const isChampion = (S, playerId) => {
  const map = getAllChampions(S);
  return !!(map[playerId] && map[playerId].length > 0);
};

export const countChampionships = (S, playerId) => {
  const map = getAllChampions(S);
  return map[playerId] ? map[playerId].length : 0;
};
...
/* ===== H2H DETAILED STATS =====
 * Rich enfrentamiento statistics between two players.
 * All numbers are computed only from matches played between these two players.
 *
 * Returns {
 *   totalMatches, w1, w2,
 *   bySurface: { Dura: {w1,w2}, ... },
 *   currentStreak: { playerId, count } | null,
 *   bestStreak1, bestStreak2,
 *   lastMatch: match object | null,
 *   lastMatchDaysAgo,
 *   tieBreaks: { w1, w2 },
 *   setsWon1, setsWon2, gamesWon1, gamesWon2,
 *   avgSetsPerMatch,
 *   longestMatch: match | null,  // by total games
 *   pointDiffNow: pointsP1 - pointsP2 (current),
 *   evolution: [{ date, w1, w2 }, ...]  for chart
 * }
 */
export const computeH2HDetails = (S, p1, p2) => {
  if (!S || !p1 || !p2 || p1 === p2) return null;
  const matches = (S.matches || [])
    .filter(
      (m) =>
        !m.annulled &&
  ...
  let tbW1 = 0,
    tbW2 = 0;
  let setsTotalCount = 0;
  let longestMatch = null;
  let longestGameCount = -1;
  const evolution = []; // [{date, w1, w2}]
  ...
  for (const m of matches) {
    ...
    // By surface
    if (!bySurface[m.surface]) bySurface[m.surface] = { w1: 0, w2: 0 };
    if (p1Won) bySurface[m.surface].w1++;
    else bySurface[m.surface].w2++;

    // Parse sets (scores are p1's score first)
    if (m.score) {
      const setParts = m.score.split(",").map((s) => s.trim());
      let matchGames = 0;
      for (const st of setParts) {
        const nums = st.split("-").map((n) => parseInt(n.trim()));
        if (nums.length !== 2 || isNaN(nums[0]) || isNaN(nums[1])) continue;
        const [a, b] = nums;
        // a is m.player1's games, b is m.player2's games
        const [p1Games, p2Games] = m.player1 === p1 ? [a, b] : [b, a];
        gamesWon1 += p1Games;
        gamesWon2 += p2Games;
        matchGames += p1Games + p2Games;
        if (p1Games > p2Games) setsWon1++;
        else if (p2Games > p1Games) setsWon2++;
        setsTotalCount++;
        // Tie-break detection: set ended 7-6
        if (
          (p1Games === 7 && p2Games === 6) ||
          (p2Games === 7 && p1Games === 6)
        ) {
          if (p1Games > p2Games) tbW1++;
          else tbW2++;
        }
      }
      if (matchGames > longestGameCount) {
        longestGameCount = matchGames;
        longestMatch = m;
      }
    }

    evolution.push({ date: m.date, w1, w2 });
  }

  // Current streak: consecutive wins by the same side ending at the last match
  let streakPlayerId = null,
    streakCount = 0;
  for (let i = matches.length - 1; i >= 0; i--) {
    const m = matches[i];
    if (streakPlayerId === null) {
      streakPlayerId = m.winner;
      streakCount = 1;
    } else if (m.winner === streakPlayerId) {
      streakCount++;
    } else {
      break;
    }
  }
  // Best historical streak of each side (within the H2H context)
  let bestStreak1 = 0,
    bestStreak2 = 0,
    curr1 = 0,
    curr2 = 0;
  for (const m of matches) {
    if (m.winner === p1) {
      curr1++;
      curr2 = 0;
      if (curr1 > bestStreak1) bestStreak1 = curr1;
    } else {
      curr2++;
      curr1 = 0;
      if (curr2 > bestStreak2) bestStreak2 = curr2;
    }
  }

  // Last match
  const lastMatch = matches[matches.length - 1];
  const lastMatchDaysAgo = lastMatch
    ? Math.floor((Date.now() - lastMatch.date) / 86400000)
    : null;

  // Current point diff (global ranking)
  const currentPts = computePointsAtDate(S, Date.now(), "all");
  const pointDiffNow = (currentPts[p1] || 0) - (currentPts[p2] || 0);
  ...
  return {
    ...
    bySurface,
    currentStreak:
      streakPlayerId && streakCount > 0
        ? { playerId: streakPlayerId, count: streakCount }
        : null,
    bestStreak1,
    bestStreak2,
    lastMatch,
    lastMatchDaysAgo,
    tieBreaks: { w1: tbW1, w2: tbW2 },
    setsWon1,
    setsWon2,
    gamesWon1,
    gamesWon2,
    avgSetsPerMatch: matches.length > 0 ? setsTotalCount / matches.length : 0,
    longestMatch,
    pointDiffNow,
    evolution,
  };
...
/* ===== YEARLY NATURAL-CALENDAR PODIUMS =====
 * Starting from 2026, compute the top-3 players per natural calendar year,
 * using the same net-balance metric as seasons (computeSeasonStandings).
 * Not archived, not closeable — pure derived data.
 */
const YEARLY_FIRST_YEAR = 2026;

export const getYearlyPodiums = (S) => {
  if (!S) return [];
  const nowDate = new Date();
  const currentYear = nowDate.getFullYear();
  if (currentYear < YEARLY_FIRST_YEAR) return [];
  const result = [];
  for (let y = YEARLY_FIRST_YEAR; y <= currentYear; y++) {
    const start = new Date(y, 0, 1, 0, 0, 0, 0).getTime();
    const end = new Date(y + 1, 0, 1, 0, 0, 0, 0).getTime() - 1;
    const standings = computeSeasonStandings(S, start, end);
    const hasActivity = standings.some(
      (r) => (r.wins || 0) + (r.losses || 0) > 0
    );
    if (!hasActivity) continue; // skip years where nothing happened
