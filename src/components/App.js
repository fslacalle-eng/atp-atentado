...
function MatchModal(){const[p1,sp1]=useState(isA?"":myId);const[p2,sp2]=useState("");const[w,sw]=useState("");const[scr,sscr]=useState("");const[dt,sdt]=useState(toI(now()));const[sf,ssf]=useState("Dura");const[bo,setBo]=useState(3);const ok=p1&&p2&&p1!==p2?canPl(p1,p2):true;const h2h=p1&&p2?hasH2H(p1,p2):false;
...
return(<Mdl title="Registrar Partido">
...
{isA&&(<><label style={slb}>Jugador 1</label><select style={{...ss,marginBottom:8}} value={p1} onChange={e=>{sp1(e.target.value);sw("");sscr("");}}><option value="">...</option>{eligibleP1.map(p=>(<option key={p.id} value={p.id}>{p.nickname}</option>))}</select></>)}
<label style={slb}>{isA?"Jugador 2":"Rival"}</label><select style={{...ss,marginBottom:8}} value={p2} onChange={e=>{sp2(e.target.value);sw("");sscr("");}} disabled={amISuspended&&!isA}><option value="">...</option>{eligibleP2.map(p=>(<option key={p.id} value={p.id}>{p.nickname+" ("+String(gP(p.id))+" pts)"}</option>))}</select>
{p1&&p2&&!ok&&<div style={{...bdg(W.warn),marginBottom:8}}>No cuenta — se registrará como Amistoso</div>}
{p1&&p2&&(<><label style={slb}>Resultado</label><div style={{marginBottom:12}}><ScoreInput p1Name={pSh(p1)} p2Name={pSh(p2)} bestOf={bo} onBestOfChange={setBo} showBestOfToggle={true} onScoreChange={s=>sscr(s)} onWinnerChange={side=>sw(side||"")}/></div></>)}
{actualW&&(<div style={{background:W.darkGreen,borderRadius:8,padding:10,marginBottom:12,border:"1px solid "+W.gold+"22",display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:12,color:W.cream3}}>{!ok?"Amistoso — no puntúa":isBonus?"Bonus equidad aplicado":"Base"}</span><span style={{fontSize:16,fontWeight:700,color:W.gold}}>{ok?(String(pp)+" pts"):"0 pts"}</span></div>)}
<button style={{...sbt("primary"),width:"100%",opacity:actualW&&!(amISuspended&&!isA)?1:.4}} disabled={amISuspended&&!isA} onClick={()=>{if(!p1||!p2||!actualW||p1===p2)return;if(amISuspended&&!isA)return;const pts=calcPts(gP(actualW),gP(actualW===p1?p2:p1),S.config.eloBase,S.config.equityBonusPct,S.config.equityMatchesGap,h2h);up(s=>({...s,matches:[...s.matches,{id:uid(),player1:p1,player2:p2,winner:actualW,score:scr,date:frI(dt),points:pts,countsForStandings:ok,isChallenge:false,tournamentId:null,annulled:false,surface:sf,status:isA?"confirmed":"pending",submittedBy:myId,reactions:{},comments:[]}]}));setModal(null);flash(isA?"Registrado":"Enviado");}}>REGISTRAR</button></Mdl>);}
...
<nav style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:20}}>{tabs.map(([id,l])=>(<button key={id} style={{...snv(view===id),position:"relative"}} onClick={()=>setView(id)}>{l}{id==="forum"&&forumHasNew&&view!=="forum"&&<span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:W.danger,marginLeft:6,verticalAlign:"middle",boxShadow:"0 0 6px "+W.danger}}/>}</button>))}</nav>
  ...
  {view==="matches"&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:18,fontWeight:700,margin:0,color:W.gold}}>Partidos</h2><button style={{...sbt("primary"),opacity:amISuspended&&!isA?0.4:1}} disabled={amISuspended&&!isA} onClick={()=>{if(amISuspended&&!isA){flash("Estás suspendido");return;}setModal("match");}}>+ Nuevo</button></div>
  ...
  {pendM.length>0&&<div style={{marginBottom:16}}><h3 style={{fontSize:14,fontWeight:600,color:W.warn,marginBottom:8}}>Pendientes de confirmacion</h3>{pendM.map(m=>matchCard(m))}</div>}{dispM.length>0&&<div style={{marginBottom:16}}><h3 style={{fontSize:14,fontWeight:600,color:W.danger,marginBottom:8}}>Disputados</h3>{dispM.map(m=>matchCard(m))}</div>}{(()=>{const filtered=[...S.matches].filter(m=>m.winner||m.score).sort((a,b)=>b.date-a.date).filter(m=>{if(mSurfF!=="all"&&m.surface!==mSurfF)return false;if(mTypeF==="all")return true;const cat=mCat(m);if(mTypeF==="regular")return cat.label==="Liga Regular";if(mTypeF==="torneo")return cat.label==="Torneo";if(mTypeF==="amistoso")return cat.label==="Amistoso";if(mTypeF==="reto")return cat.label==="Reto";return true;});return filtered.length>0?filtered.map(m=>matchCard(m)):<div style={{...scd,color:W.cream3,fontSize:13,fontStyle:"italic",textAlign:"center"}}>No hay partidos con estos filtros.</div>;})()}</div>)}
